...
def write_one(file: pathlib.Path, url: str, session: requests.Session) -> None:
    """
...
def start_task(file: pathlib.Path, urls: set) -> None:
    ...
    with requests.Session() as session:
        ...
        threads = []
        for url in urls:
            thread = threading.Thread(target=write_one, args=(file, url, session), daemon=True)
...
def runner(quantity, plot_fig=False, save_fig=False) -> float:
    ...
    logger.info('Среднее время работы (http запросы по потокам): ' + str(mean_time))
    ...
    plt.title(f'Время выполнения запросов (по потокам), среднее - {round(mean_time,2)} сек.')  # Название
    ax.set_ylim([0, 5])
    ax.set_xlim([1, quantity])
    if save_fig:
        path_fig = here.joinpath("save_fig")
        path_fig = path_fig.joinpath("site_thread.jpg")
