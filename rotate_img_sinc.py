"""Синхронный поворот изображения"""
...
def runner(quantity, plot_fig=False, save_fig=False) -> float:
    ...
    logger.info('Среднее время поворота изображения (синхронно): ' + str(mean_time))
    ...
    plt.title(f'Время поворота изображения (синхронно), среднее - {round(mean_time, 2)} сек.')  # Название
    ax.set_ylim([0, 5.0])
    ...
    if save_fig:
        ...
        path_fig = path_fig.joinpath("rotate_sinc.jpg")
