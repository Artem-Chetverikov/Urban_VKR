"""Многопрцессорный запрос HTML страниц"""
...
def write_one(url: str, file: pathlib.Path, session: requests.Session) -> None:
    """
...
def start_task(file: pathlib.Path, urls: set, pool: multiprocessing.Pool) -> None:
    """
    ...
    with requests.Session() as session:
        ...
        write_one_ = partial(write_one, file=file, session=session)
        pool.map(write_one_, urls)
...
def runner(quantity, plot_fig=False, save_fig=False) -> float:
    ...
    for _ in range(quantity):
        ...
        start_task(file=outpath, urls=urls, pool=pool)
    ...
    logger.info('Среднее время работы (http запросы по процессам): ' + str(mean_time))
    ...
    if save_fig:
        ...
        path_fig = path_fig.joinpath("site_multiproc.jpg")
