...
def start_task(file_lst: list) -> None:
    """
    ...
    for file_i in file_lst:
        thread = threading.Thread(target=rotate_image, args=(file_i,))
...
def runner(quantity, plot_fig=False, save_fig=False) -> float:
    ...
    logger.info('Среднее время поворота изображения (по потокам): ' + str(mean_time))
    ...
    plt.title(f'Время поворота изображения (по потокам), среднее - {round(mean_time, 2)} сек.')  # Название
    ax.set_ylim([0, 5.0])
    ...
    if save_fig:
        ...
        path_fig = path_fig.joinpath("rotate_thread.jpg")
