...
def start_task(file_lst: list, pool: multiprocessing.Pool) -> None:
    """
    ...
    :return:
    """
    pool.map(rotate_image, file_lst)
...
def runner(quantity, plot_fig=False, save_fig=False) -> float:
    ...
    logger.info('Среднее время поворота изображения (по процессам): ' + str(mean_time))
    ...
    plt.title(f'Время поворота изображения (по процессам), среднее - {round(mean_time, 2)} сек.')  # Название
    ax.set_ylim([0, 5.0])
    ...
    if save_fig:
        ...
        path_fig = path_fig.joinpath("rotate_multiproc.jpg")
