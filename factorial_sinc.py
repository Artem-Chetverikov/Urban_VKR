"""Синхронное вычисление факториала"""
...
def runner(quantity, plot_fig=False, save_fig=False) -> float:
    ...
    logger.info('Среднее время вычисления факториала (синхронно): ' + str(mean_time))
    ...
    plt.title(f'Время вычисления факториала (синхронно), среднее - {round(mean_time, 2)} сек.')  # Название
    ax.set_ylim([0, 3.0])
    ...
    if save_fig:
        ...
        path_fig = path_fig.joinpath("factorial_sinc.jpg")
